...
function isDatabaseConflictError(e) {
  ...
  return typeof message === "string" && (/SQLITE_CONSTRAINT(?:_UNIQUE)?: UNIQUE/.test(message) || /unique constraint/.test(message));
}
...
const parseURL$1 = (url, config) => {
  ...
  if (config.s3ForcePathStyle) {
    ...
    if (host.indexOf(".") < 0) {
      throw new Error(`invalid virtual hosted-style AWS S3 URL, ${url} does not contain bucket prefix in the host`);
    }
    [bucket] = host.split(".");
  ...
  if (config.host === "amazonaws.com") {
    ...
    if (!match) {
      throw new Error(`invalid AWS S3 URL, cannot parse region from host in ${url}`);
    }
    region = match[1];
  } else {
    region = "";
...
AwsS3UrlReader.factory = ({ config, treeResponseFactory }) => {
  ...
  return integrations.awsS3.list().map((integration) => {
    const creds = _AwsS3UrlReader.buildCredentials(integration);
    const s3 = new aws.S3({
