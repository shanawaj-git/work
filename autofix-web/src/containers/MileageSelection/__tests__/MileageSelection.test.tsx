...
import MileageSelection, { pageName } from "..";
import { withWrapper } from "@/utils/IntlProviderWrapper";
import { useRouter } from "next/router";

const mockConfig = {
  "mileage.selection.range.start": "0",
  "mileage.selection.range.stop": "200000",
  "mileage.selection.range.step": "1000",
};

const totalNumberMenuItems = 201; //  for given mockConfig

const renderComponentWithMockData = () => {
  return render(withWrapper(<MileageSelection config={mockConfig} />));
};

describe("<MileageSelection />", () => {
  ...
  test("when proper config provided, should render horizontal scroll menu", () => {
    ...
    expect(check.innerHTML).toBe("0");
  });

  test("when user clicks on continue button, should redirect to car-confirmation route", () => {
    const expectedRouteOptions = {
      pathname: "/car-overview",
      query: { mileage: "0" },
    };
    const tree = renderComponentWithMockData();
  ...
  it("when user clicks on horizontal scroll item-10000, it should be selected and display in title", async () => {
    const tree = renderComponentWithMockData();
    const element = tree.getByTestId("scrollable-menu-item-10000");
    fireEvent(element, new MouseEvent("click", { bubbles: true }));
    tree.rerender(withWrapper(<MileageSelection config={mockConfig} />));
    const check = await waitFor(() =>
      tree.getByTestId("horizontal-scroll-menu-title")
    );

    expect(check.innerHTML).toBe("10000");
  });
});
